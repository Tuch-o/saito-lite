...
class Crypto {
   ...
   */
  hash(data="") { return SHA256(data).toString(CryptoJS.enc.Hex); }
   ...
   * of SHA256 hashes.
   * @param {String[]} inputs array of strings
   * @returns {Object}
   */
  returnMerkleTree(inputs) {
    var tree;
    try {
      tree = new MerkleTree(inputs, SHA256);
    } catch(err) {
      tree = new MerkleTree.default(inputs, SHA256);
    }
    return tree;
  }
